# 📥 DownloaderMD

**Descargador universal para YouTube, MP3 y links directos**  
Funciona en PC y Android con Pydroid 3

![Python](https://img.shields.io/badge/Python-3.8+-blue)
![License](https://img.shields.io/badge/License-MIT-green)
![Version](https://img.shields.io/badge/Version-1.0-brightgreen)

### ✨ Características
- **YouTube**: Descarga Video en 1080p, 720p, 480p, 360p
- **MP3**: Extrae solo el audio de YouTube
- **Requests**: Descarga cualquier link directo
- **Multiplataforma**: no, todavia es solamente compatible con windows.
- **Interfaz moderna**: Estilo Whatsapp.

### 📲 Descarga
Ve a la sección **[Releases](https://github.com/Doriogamer/DownloaderMD/releases)** y descarga `installer.exe`

### 🛠️ Requisitos
**PC:** Windows 10+

### 🚀 Como usar
1. Pega el link de YouTube o descarga directa
2. Elige Video o MP3 (en el apartado de configuración.)
3. Selecciona la calidad (en el apartado de configuración.)
4. Dale a DESCARGAR

### 📸 Capturas
*Pronto subiré capturas de la app*

### 🤝 Contribuir
Acepto ideas y mejoras. Abre un "Issue" o "Pull Request"

### ⭐ Si te sirvió el proyecto dame una estrella!
